...
import DataTable, { IconEdit, IconEye, IconTrash, type Action, type Column, type FilterConfig } from './table';
...
const columns: Column<Funcionario>[] = [
  ...
  { key: 'departamento', label: 'Departamento' },  
  {
    key: 'criado_em',
    label: 'Criado em',
    render: (row: Funcionario) => row.criado_em ? new Date(row.criado_em).toLocaleDateString('pt-PT') : '-',
  },
];
...
const filterConfig: FilterConfig<Funcionario>[] = [
  {
    ...
    options: [
      ...
      { label: 'Analista Programador', value: 'Analista Programador' },
...
export default function ListarFuncionario() {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/funcionarios`, {
      ...
      .then((json) => {
        // Monitorizacao
        console.log("Dados recebidos da API SGF:", json);
  ...
  const actions: Action<Funcionario>[] = [
    {
      icon: <IconEye />,
      label: 'Visualizar',
      onClick: (row) => navigate(`/dashboard/funcionario/${row.id}`), 
    },
    {
      icon: <IconEdit />,
      label: 'Editar',
      onClick: (row) => navigate(`/dashboard/editar-funcionario/${row.id}`), 
    },
    {
      icon: <IconTrash />,
      label: 'Eliminar',
        ...
        <div style={modalOverlayStyle}>
          <div style={modalContentStyle}>
            ...
            <p style={{ color: '#444', fontSize: '14px', lineHeight: '1.5' }}>
              Tens a certeza de que desejas eliminar o funcionário <strong>{selectedFuncionario.nome}</strong>? Esta acção não poderá ser desfeita.
            </p>
            <div style={modalActionsStyle}>
              <button onClick={closeDeleteDialog} disabled={isDeleting} style={cancelButtonStyle}>Cancelar</button>
              <button onClick={confirmDelete} disabled={isDeleting} style={deleteButtonStyle}>
